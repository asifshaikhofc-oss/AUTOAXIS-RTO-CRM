...
def dashboard():
    ...
    conn.close()
    return render_template('dashboard.html', entries=entries, total_count=total_count, pending_count=pending_count, in_process_count=in_process_count, objection_count=objection_count, done_count=done_count, finance_stats=finance_stats, user=session['user'], role=session['role'])
...
def add_entry():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('dashboard'))
    return render_template('add.html', role=session['role'])
...
def edit_entry(id):
    ...
    if session['role'] == 'admin':
        # ⚡ BACKUP FALLBACK LOGIC FIXED ⚡
        # Agar koi field blank mile toh purani saved database value uthao, 0.0 mat karo!
        c_amt = request.form.get('challan_amt')
        c_amt = float(c_amt) if (c_amt and c_amt.strip() != '') else float(entry['challan_amt'] or 0.0)
        
        p_amt = request.form.get('puc_amt')
        p_amt = float(p_amt) if (p_amt and p_amt.strip() != '') else float(entry['puc_amt'] or 0.0)
        
        i_amt = request.form.get('insurance_amt')
        i_amt = float(i_amt) if (i_amt and i_amt.strip() != '') else float(entry['insurance_amt'] or 0.0)
        
        t_bill = request.form.get('total_billing')
        t_bill = float(t_bill) if (t_bill and t_bill.strip() != '') else float(entry['total_billing'] or 0.0)
        
        r_amt = request.form.get('received_amt')
        r_amt = float(r_amt) if (r_amt and r_amt.strip() != '') else float(entry['received_amt'] or 0.0)
        
        # Auto-sum fallback engine
        if t_bill == 0.0:
            t_bill = c_amt + p_amt + i_amt
...
def print_pdf():
    ...
    for r in raw_rows:
        e = dict(r)
        e['total_billing'] = float(e['total_billing'] or 0.0)
        e['received_amt'] = float(e['received_amt'] or 0.0)
        e['balance_amt'] = float(e['balance_amt'] or 0.0)
        entries.append(e)
    
    summary_query = "SELECT SUM(COALESCE(total_billing,0)) as total_bill, SUM(COALESCE(received_amt,0)) as total_recv, SUM(COALESCE(total_billing,0) - COALESCE(received_amt,0)) as total_bal FROM entries WHERE 1=1"
    if search_query:
        summary_query += " AND (vehicle_no LIKE ? OR owner_name LIKE ? OR dealer LIKE ?)"
    summary = conn.execute(summary_query, params).fetchone()
    conn.close()
    
    return render_template('pdf_report.html', entries=entries, search_query=search_query, summary=summary, role=session['role'], invoice_no="AXS-"+datetime.now().strftime("%Y%m%d")+"-01")

@app.route('/hold_cases')
def hold_cases():
    if 'user' not in session: return redirect(url_for('home'))
    conn = get_db_connection()
    raw_entries = conn.execute("SELECT * FROM entries WHERE status = 'PENDING' OR status = 'HOLD' ORDER BY id DESC").fetchall()
    hold_entries = []
    current_date = datetime.now()
    for row in raw_entries:
        entry = dict(row)
        if entry['created_date']:
            try:
                created_dt = datetime.strptime(entry['created_date'], '%d-%m-%Y')
                days_diff = (current_date - created_dt).days
                entry['days_old'] = days_diff
                if days_diff > 60: entry['status'] = 'HOLD'; hold_entries.append(entry)
            except: pass
    conn.close()
    return render_template('hold_report.html', entries=hold_entries, user=session['user'], role=session['role'])

@app.route('/submit_file/<int:id>')
def submit_file(id):
    if 'user' not in session: return redirect(url_for('home'))
    today_date = datetime.now().strftime("%d-%m-%Y")
    conn = get_db_connection()
    conn.execute("UPDATE entries SET status = 'IN PROCESS', submission_date = ? WHERE id = ?", (today_date, id))
    conn.commit(); conn.close()
    return redirect(url_for('dashboard'))
...
def export_csv():
    ...
    search_query = request.args.get('search', '').strip().upper()
    conn = get_db_connection()
    query = "SELECT vehicle_no, vehicle_model, owner_name, work_type, dealer, submission_date, challan_amt, total_billing, received_amt, (COALESCE(total_billing,0) - COALESCE(received_amt,0)) as balance, status, remarks FROM entries WHERE 1=1"
    params = []
    if search_query:
        query += " AND (vehicle_no LIKE ? OR owner_name LIKE ? OR dealer LIKE ?)"
        like_str = f"%{search_query}%"
    ...
    if session['role'] == 'admin':
        writer.writerow(['VEHICLE NO', 'MODEL', 'CLIENT NAME', 'WORK TYPE', 'DEALER NAME', 'PROCESS DATE', 'CHALLAN (₹)', 'TOTAL BILLING (₹)', 'RECEIVED (₹)', 'BALANCE (₹)', 'STATUS', 'REMARKS'])
        for r in rows: writer.writerow([r['vehicle_no'], r['vehicle_model'], r['owner_name'], r['work_type'], r['dealer'], r['submission_date'], r['challan_amt'], r['total_billing'], r['received_amt'], r['balance'], r['status'], r['remarks']])
    else:
        writer.writerow(['VEHICLE NO', 'MODEL', 'CLIENT NAME', 'WORK TYPE', 'DEALER NAME', 'PROCESS DATE', 'CHALLAN (₹)', 'STATUS', 'REMARKS'])
        for r in rows: writer.writerow([r['vehicle_no'], r['vehicle_model'], r['owner_name'], r['work_type'], r['dealer'], r['submission_date'], r['challan_amt'], r['status'], r['remarks']])
    response = Response(output.getvalue(), mimetype="text/csv")
    response.headers["Content-Disposition"] = f"attachment; filename=Autoaxis_Report_{datetime.now().strftime('%d-%m-%Y')}.csv"
    return response
